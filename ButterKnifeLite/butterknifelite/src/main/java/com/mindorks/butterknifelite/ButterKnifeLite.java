...
public class ButterKnifeLite {
    ...
    public static void bind(final Activity target) {
        ...
        bindViewsWithDefaultData(target, target.getClass().getDeclaredFields(), target.findViewById(android.R.id.content));
    ...
    public static void bind(final Object obj, View promptsView){
        ...
        bindViewsWithDefaultData(obj, obj.getClass().getDeclaredFields(), promptsView);
    ...
    /**
     * initiate the view for the annotated public fields with default data
     *
     * @param obj      is any class instance with annotations
     * @param fields   list of methods in the class with annotation
     * @param rootView is the inflated view from the XML
     */
    private static void bindViewsWithDefaultData(final Object obj, Field[] fields, View rootView) {
        // Iterating over the fields

        for (final Field field : fields) {
            Annotation annotation = field.getAnnotation(BindViewWithData.class);

            if (annotation != null) {

                BindViewWithData bindView = (BindViewWithData) annotation;
                int id = bindView.value();
                String data = bindView.defaultData();
                View view = rootView.findViewById(id);
                try {
                    field.setAccessible(true);
                    field.set(obj, view);
                    // Check if it is a TextView
                    if (view instanceof TextView || view instanceof Button
                            || view instanceof EditText) ((TextView) view).setText(data);
                } catch (IllegalAccessException e) {
                    e.printStackTrace();
                }
            }
